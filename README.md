...
当前版本已根据熊天煜的经历整理为个人主页，默认不展示头像，也没有使用背景照片。

### 添加头像

1. 将头像放到 `static/assets/img/`，例如命名为 `avatar.jpg`。
2. 在 `index.html` 中搜索“暂不展示头像”，将该注释替换为：

```html
<div class="container px-4 px-lg-5 avatar-wrap">
  <img class="avatar" src="static/assets/img/avatar.jpg" alt="熊天煜的头像">
</div>
```

3. 在 `static/css/main.css` 末尾添加：
...
.avatar {
  ...
  box-shadow: 0 18px 45px rgba(16, 29, 61, 0.22);
}

@media (max-width: 767px) {
  .avatar { width: 112px; height: 112px; border-radius: 18px; }
}
```

建议使用正方形图片，`object-fit: cover` 会避免头像变形。

### 使用背景照片

1. 将背景图放到 `static/assets/img/`，例如命名为 `hero-background.jpg`。
2. 在 `static/css/main.css` 中找到 `.top-section` 的 `background`，改为：

```css
background:
  linear-gradient(rgba(16, 29, 61, 0.68), rgba(24, 59, 122, 0.72)),
  url("../assets/img/hero-background.jpg") center / cover no-repeat;
```

渐变层用于保证文字在照片上仍然清晰；可调整两个 `rgba` 的最后一个数值改变遮罩深浅。
